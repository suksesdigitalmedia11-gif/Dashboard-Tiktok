...
const DEFAULT_STORES = ["ventura", "custombase", "giftyours", "cetakin", "memora", "piksera", "tempelin"];
const DEFAULT_STORE = "ventura";
const CALCULATION_VERSION = "custombase-finance-v1";
...
// ── Special SKU HPP mapping (hardcoded, variation-based) ──
// Dipakai sebagai fallback terakhir jika DB tidak punya HPP untuk SKU tsb.
// Formula polaroidnew: HPP = (pcs / 25) × 3000 × quantity
// Packing = 2000 × quantity (per paket, dikali qty di caller)

// SKU-SKU yang HPP-nya tergantung variation quantity
const POLAROID_SKU_TOKENS = new Set(['polaroidnew', 'pola', 'polaroid', 'polaroidnewpcs']);

// Ekstrak jumlah pcs dari variation string (misal "25pcs", "50 Pcs", "100 pcs" → angka)
function extractPcsFromVariation(variation) {
  if (!variation) return 0;
  const varToken = String(variation).toLowerCase().replace(/[^a-z0-9]+/g, '');
  const match = varToken.match(/(\d+)pcs/);
  return match ? parseInt(match[1], 10) : 0;
}

// HPP per unit (per order line, sebelum kali qty) untuk polaroid SKU
// Returns 0 jika bukan polaroid SKU atau pcs tidak dikenali
function getPolaroidHpp(pcs) {
  if (pcs <= 0 || pcs % 25 !== 0) return 0; // hanya 25, 50, 100, 125, 150, 200
  const factor = pcs / 25;
  return factor * 3000;
}

// Packing per unit (per order line, sebelum kali qty) untuk polaroid SKU
function getPolaroidPacking(pcs) {
  if (pcs <= 0) return 0;
  return 2000; // Rp2.000 per paket, apapun ukurannya
}

function getSpecialSkuHpp(skuToken, variation) {
  // Cek apakah termasuk polaroid SKU
  if (POLAROID_SKU_TOKENS.has(skuToken)) {
    const pcs = extractPcsFromVariation(variation);
    const hpp = getPolaroidHpp(pcs);
    if (hpp > 0) return hpp;
  }
  // Legacy canonical HPP (non-variation based)
  const CANONICAL_FIXED = {
    'gancifotonama': 2000,
    'holologo9': 9000,
    'ganciksgn1': 1500,
  };
  return CANONICAL_FIXED[skuToken] || 0;
}

function getSpecialSkuPacking(skuToken, variation) {
  if (POLAROID_SKU_TOKENS.has(skuToken)) {
    const pcs = extractPcsFromVariation(variation);
    return getPolaroidPacking(pcs);
  }
  return 0;
}
...
function parseDate(value) {
  ...
  if (slash) {
    ...
    // Heuristic: if first > 12, it's DD/MM. If second > 12, it's MM/DD.
    // If both <= 12, prefer DD/MM (Indonesian standard).
    const isDDMM = a > 12 || b <= 12;
    const day = (isDDMM ? slash[1] : slash[2]).padStart(2, "0");
    const month = (isDDMM ? slash[2] : slash[1]).padStart(2, "0");
...
function detectKind(rows, requestedKind) {
  ...
  // TikTok Ad Transaction file (EN & ID bahasa)
  if (
    has("Transaction time", "transaction time", "Waktu transaksi", "waktu transaksi") &&
    has("Transaction type", "transaction type", "Tipe transaksi", "tipe transaksi") &&
    has("Status", "status")
  ) return "ads";
  if (
...
function mapOrderRows(rows, storeName, filename) {
  ...
  return rows.map(row => {
    ...
    // Seller SKU bisa kosong untuk produk yang tidak dikonfigurasi seller SKU-nya di TikTok.
    // Fallback ke Platform SKU ID (kolom 'SKU ID') agar order tidak ter-skip.
    const sellerSku = String(field(row, "SKU Marketplace", "Seller SKU")).trim();
    const platformSkuId = String(field(row, "SKU ID", "Platform SKU ID", "platform_sku_id")).trim();
    const sku = sellerSku || platformSkuId;
...
async function finishImportRun(runId, patch) {
  ...
  // Triple-mode compatible: pakai ? placeholder (normalizeSql konversi $N→? di SQLite)
  const sets = cols.map(c => '"' + c + '" = ?').join(', ');
  const values = [...cols.map(c => patch[c]), runId];
  try {
    const result = await pg.pgQuery(
      `UPDATE "finance_import_runs" SET ${sets} WHERE "id" = ?`,
      values
    );
    return (result.rows || [])[0] || null;
  } catch (e) {
    console.error('[importRun] finishImportRun failed:', e.message);
    return null;
  }
}
...
async function importIncomeRows({ storeName, filename, rows, run }) {
  ...
  // === SIMPAN income_raw menggunakan upsertRows (triple-mode safe) ===
  if (rawIncomeRows.length > 0) {
    try {
      await pg.upsertRows('finance_income_raw', rawIncomeRows, ['store_name', 'order_id', 'transaction_type']);
    } catch (e) {
      // Fallback: insert row by row jika upsertRows gagal
      console.error('[importIncome] upsertRows income_raw failed:', e.message);
      for (const ri of rawIncomeRows) {
        try {
          await pg.pgQuery(
            'INSERT INTO finance_income_raw(store_name,transaction_type,order_id,order_created_time,settlement_amount,total_fees,refund_amount,adjustment_amount,imported_at) VALUES(?,?,?,?,?,?,?,?,?)',
            [ri.store_name, ri.transaction_type, ri.order_id, ri.order_created_time, ri.settlement_amount, ri.total_fees, ri.refund_amount, ri.adjustment_amount, ri.imported_at]
          );
        } catch (e2) { /* skip dup */ }
      }
    }
  ...
  // Audit events: skip for Turso to avoid 28K+ HTTP roundtrips (data integrity unaffected)
  if (auditEvents.length > 0) {
    try { await insertRows("finance_audit_events", auditEvents); } catch (e) { }
  }
  ...
  return {
    ok: true, kind: "income", rows: rows.length, inserted, updated, unchanged: unchanged + skipped, skipped, auditCount: auditEvents.length, adSpendRows, adSpendTotal,
...
async function importRows({ storeName, kind, filename, rows }) {
  ...
  if (detected === "ads") {
    // TikTok ad transaction file: extract BOTH Add balance (top-up) AND Bill payment (GMV)
    const adRows = [];
    let gmvCount = 0, gmvTotal = 0, topUpCount = 0, topUpTotal = 0;
    const seenTxIds = new Set();
    for (const row of cleanRows) {
      // Support kolom Bahasa Indonesia (Tipe transaksi, Subjenis transaksi, dll)
      const type = String(field(row, "Transaction type", "Tipe transaksi") || "").trim();
      const subtype = String(field(row, "Transaction subtype", "Subjenis transaksi") || "").trim();
      const status = String(field(row, "Status") || "").trim();
      const txId = String(field(row, "Transaction ID", "ID Transaksi") || "").trim();
      // Accept status: Success (EN) / Berhasil (ID)
      if (status !== "Success" && status !== "Successful" && status !== "Berhasil" && status !== "berhasil") continue;
      if (txId && seenTxIds.has(txId)) continue;
      if (txId) seenTxIds.add(txId);

      const rawDate = field(row, "Transaction time", "Waktu transaksi") || "";
      let spendDate;
      if (rawDate instanceof Date && !isNaN(rawDate)) {
        spendDate = rawDate.toISOString().slice(0, 10);
      } else {
      ...
      // Support kolom Jumlah (ID) dan Amount (EN)
      let amount = Math.abs(rupiah(field(row, "Amount", "Jumlah") || 0));
      if (!amount) amount = Math.abs(rupiah(field(row, "Total Amount", "GMV", "Nominal") || 0));
      if (!amount) continue;

      const description = String(field(row, "Description", "Deskripsi") || "").trim();
      const fundType = String(field(row, "Fund type", "Jenis dana") || "").trim();
      const subtypeToken = columnToken(subtype);

      // ── Top-up / Add balance (EN: Add balance | ID: Tambahkan saldo) ──
      if (subtypeToken === "addbalance" || subtypeToken.includes("topup") || subtypeToken.includes("addcredit") ||
        subtypeToken === "tambahkansaldo" || subtypeToken.includes("tambahsaldo") || subtypeToken.includes("tambahkansaldo")) {
        adRows.push({
          store_name: normalizeStore(storeName || DEFAULT_STORE),
          spend_date: spendDate, amount,
          channel: "TikTok Top Up", campaign: "Manual Top Up",
          note: ("Top up " + (txId ? "(" + txId + ")" : "") + " " + description).trim(),
          transaction_id: txId || null,
          status: status,
          created_at: nowIso(), updated_at: nowIso(),
        });
        topUpCount++; topUpTotal += amount;
      }
      // ── GMV Max / Bill Payment (auto-deducted) ──
      else if (subtypeToken === "billpayment" || subtypeToken.includes("gmv") || subtypeToken.includes("campaignresource")) {
        adRows.push({
          store_name: normalizeStore(storeName || DEFAULT_STORE),
          spend_date: spendDate, amount,
          channel: "TikTok GMV Ads", campaign: subtype || "GMV Auto",
          note: ("GMV: " + (description || txId)).trim(),
          transaction_id: txId || null,
          status: status,
          created_at: nowIso(), updated_at: nowIso(),
        });
        gmvCount++; gmvTotal += amount;
      }
      // ── Campaign / Affiliate ads ──
      else if (subtypeToken.includes("campaign") || subtypeToken.includes("affiliate") || subtypeToken.includes("shopads")) {
        adRows.push({
          store_name: normalizeStore(storeName || DEFAULT_STORE),
          spend_date: spendDate, amount,
          channel: "TikTok Ads Settlement", campaign: subtype || description || "Ads",
          note: ((description || subtype) + " (" + txId + ")").trim(),
          transaction_id: txId || null,
          status: status,
          created_at: nowIso(), updated_at: nowIso(),
        });
      ...
      // ── Issued = kredit/voucher yang diberikan TikTok ke akun iklan.
      // Ini MENGURANGI spend efektif, simpan sebagai NEGATIF agar net spend benar.
      else if (subtypeToken === "issued" || subtypeToken.includes("voucher") || subtypeToken.includes("credit")) {
        adRows.push({
          store_name: normalizeStore(storeName || DEFAULT_STORE),
          spend_date: spendDate, amount: -amount, // NEGATIF = kredit/pengurangan spend
          channel: "TikTok GMV Ads", campaign: subtype || "Issued Credit",
          note: ("Kredit iklan: " + (description || txId)).trim(),
          transaction_id: txId || null,
          status: status,
          created_at: nowIso(), updated_at: nowIso(),
        });
        // Tidak dihitung ke gmvCount/gmvTotal karena ini pengurangan
      }
    }
    if (adRows.length) await upsertAdSpendRows(adRows);
    const adMsg = "Iklan diproses: " + adRows.length + " transaksi — Top Up " + topUpCount + " (Rp" + Math.round(topUpTotal).toLocaleString("id-ID") + "), GMV Ads " + gmvCount + " (Rp" + Math.round(gmvTotal).toLocaleString("id-ID") + ")";
    await finishImportRun(run && run.id, { rows_seen: cleanRows.length, inserted: adRows.length, updated: 0, unchanged: 0, audit_count: 0, message: adMsg });
    return { ok: true, kind: "ads", rows: cleanRows.length, inserted: adRows.length, updated: 0, unchanged: 0, auditCount: 0, adSpendRows: adRows.length, adSpendTotal: adRows.reduce((s, r) => s + r.amount, 0), topUpTotal, gmvTotal };
  }
...
async function saveConfig(data) {
  ...
  await upsertRows("finance_config", [{ key: "app", value: JSON.stringify(next), updated_at: nowIso() }], "key");
...
async function computeSummary(filters = { preset: "thisMonth", month: "", store: "all", mode: "accrual" }) {
  ...
  // ── Check cache ──
  const cacheKey = `summary:${filters.preset}:${filters.month || ''}:${filters.store}:${filters.mode || 'accrual'}`;
  const cached = await pg.cacheGet(cacheKey);
  if (cached) {
    safeLog("summary_cache_hit", { key: cacheKey, ms: Date.now() - startedAt });
    cached.generatedAt = nowIso();
    cached.diagnostics.cached = true;
    cached.diagnostics.summaryMs = Date.now() - startedAt;
    return cached;
  }
  ...
  const incomeMatchedOrderIds = new Set();  // Order IDs that have Pesanan match in income_raw
  for (const ir of incomeRawRows) {
    ...
    if (type === "Pesanan") {
      ...
      const oid = String(ir.order_id || "").trim();
      if (oid) incomeMatchedOrderIds.add(oid);
  ...
  // Build SKU cost map — global SKU berlaku untuk semua toko (cross-store lookup).
  // Priority: store-specific > global. Match by SKU name saja (ignore product name).
  const costsStoreSpecific = new Map();
  const costsGlobal = new Map();
  for (const row of skuCosts) {
    const skuLower = String(row.sku || "").toLowerCase();
    const storeLower = String(row.store_name || "").toLowerCase();
    if (storeLower === "global" || storeLower === "" || storeLower === "all") {
      if (!costsGlobal.has(skuLower)) costsGlobal.set(skuLower, row);
    } else {
      const storeKey = `${storeLower}|${skuLower}`;
      if (!costsStoreSpecific.has(storeKey)) costsStoreSpecific.set(storeKey, row);
    }
  }
  // Also build cross-store fallback: if SKU exists in any store, use its HPP for unknown stores
  const costsAnySku = new Map();
  for (const row of skuCosts) {
    const skuLower = String(row.sku || "").toLowerCase();
    if (!costsAnySku.has(skuLower) && Number(row.hpp_per_unit || 0) > 0) {
      costsAnySku.set(skuLower, row);
    }
  }
  ...
  const rows = orderLines.map(row => {
    ...
    const storeLower = String(row.store_name || "").toLowerCase();
    const storeKey = `${storeLower}|${skuLower}`;
    // Lookup priority: store-specific → global → any-store → special hardcoded
    const cost = costsStoreSpecific.get(storeKey) || costsGlobal.get(skuLower) || costsAnySku.get(skuLower) || {};
    const dbHpp = Number(cost.hpp_per_unit || 0);
    const dbPacking = Number(cost.packing_per_unit || 0);
    // Fallback ke special SKU HPP jika DB kosong (polaroidnew variation-based)
    ...
    // Packing: DB dulu, lalu special fallback (polaroidnew = 2000/paket)
    const specialPacking = getSpecialSkuPacking(skuToken, row.variation);
    const packingPerUnit = dbPacking > 0 ? dbPacking : specialPacking;
    return {
      ...row,
      hpp_per_unit: hppPerUnit,
      packing_per_unit: packingPerUnit,
      _hasCost: dbHpp > 0 || specialHpp > 0,
      _hasPacking: dbPacking > 0,
      _unitHpp: hppPerUnit,
    };
  });
  ...
  const totals = {
    ...
    heldOrders: new Set(),
    pendingSettlementOrders: new Set(),
    pendingSettlementOmzet: 0,
  ...
  for (const [groupKey, orderRows] of groups.entries()) {
    ...
    // Pre-compute gross/discount untuk drilldown dana tertahan
    const _grossSum = operationRows.reduce((sum, row) => sum + Math.abs(Number(row.gross_product || 0)), 0);
    const _discSum = operationRows.reduce((sum, row) => sum + Math.abs(Number(row.seller_discount || 0)), 0);
    operationDetails.push({
      ...
      grossOmzet: Math.round(_grossSum),
      sellerDiscount: Math.round(_discSum),
      omzet: Math.round(Math.max(_grossSum - _discSum, 0)), // net dari product
      // Estimasi settlement = omzet net dikurangi platform fee per order (approx ~5%)
      // Angka pasti baru diketahui saat TikTok cairkan; ini estimasi saja
      estimasiSettlement: 0, // diupdate di bawah setelah platformFee dihitung
      late,
      ageDays,
      cancelValid,
      pendingSettlement: false, // diupdate di bawah jika applicable
    });
    ...
    // isShipped: hanya status "Dikirim" (bukan "Perlu dikirim", bukan "Belum dibayar")
    const isShipped = statusToken2 === "dikirim" || statusToken2.includes("shipped");
    // Explicitly exclude non-eligible statuses
    const isExcluded = statusToken2.includes("perlu") || statusToken2.includes("belum") || statusToken2.includes("batal") || statusToken2.includes("cancel");
    const revenueEligible = calcMode === "settlement" ? effectiveCompleted : (effectiveCompleted || (isShipped && !isExcluded));
    const costEligible = calcMode === "settlement" ? (effectiveCompleted || cancelValid) : (effectiveCompleted || (isShipped && !isExcluded) || cancelValid);
    ...
    // Pending settlement: completed/shipped, no settlement received, no income match
    const hasSettlement = Number(first.settlement_received || 0) > 0 || settlementTotal > 0;
    const hasIncomeMatch = incomeMatchedOrderIds.has(String(first.order_id || orderId || "").trim());
    if (!opCancelOnly && !opUnpaid && (isCompleted || isShipped) && !hasSettlement && !hasIncomeMatch) {
      totals.pendingSettlementOrders.add(orderId);
      totals.pendingSettlementOmzet += orderTotal;
      // Flag pendingSettlement di operationDetails — estimasiSettlement dihitung di bawah
      if (operationDetails.length > 0) {
        operationDetails[operationDetails.length - 1].pendingSettlement = true;
      }
    }
    ...
    // Setelah rawPlatformFeeTotal diketahui: update estimasiSettlement untuk order pending
    if (operationDetails.length > 0) {
      const lastOp = operationDetails[operationDetails.length - 1];
      if (lastOp.pendingSettlement && lastOp.estimasiSettlement === 0) {
        const estFee = rawPlatformFeeTotal > 0 ? rawPlatformFeeTotal : Math.round(orderTotal * 0.05);
        lastOp.estimasiSettlement = Math.max(Math.round(orderTotal - estFee), 0);
      }
    }
    ...
    // PACKING = biaya packaging per PAKET (per nomor order unik), BUKAN per unit produk.
    // Logika bisnis: 1 nomor order = 1 paket fisik = 1 biaya packing, meski ada 3-5 SKU di dalamnya.
    // Cara hitung: ambil packing_per_unit dari SKU pertama yang punya data (sebagai flat rate per order).
    // Fallback: Rp2.000 per paket jika tidak ada data di DB.
    const firstPackingRow = financialRows.find(r => Number(r.packing_per_unit || 0) > 0);
    const packingRatePerOrder = firstPackingRow ? Number(firstPackingRow.packing_per_unit) : 2000;
    const packingTotal = packageCount * packingRatePerOrder;
    ...
    for (const row of financialRows) {
      ...
      // Packing per-row: gunakan packing_per_unit×qty jika ada di DB,
      // fallback: share proporsional dari packingTotal order berdasarkan gross
      const rowPackingUnit = Number(row.packing_per_unit || 0);
      const packing = costBurned
        ? (rowPackingUnit > 0 ? qty * rowPackingUnit : packingTotal * (grossSum ? lineGross / grossSum : 1 / Math.max(financialRows.length, 1)))
        : 0;
      const profit = omzet - platformFee - refund - hpp - packing;
      totals.qty += qty;
      totals.hpp += hpp;
      // packing per-row sudah dihitung, total packing per-order tetap dari blok atas
  ...
  // CORRECT FORMULA: ABS(SUM(signed total_fees)), NOT SUM(ABS(total_fees))
  if (incomeRawRows && incomeRawRows.length > 0) {
    ...
    for (const ir of incomeRawRows) {
      ...
      const f = Number(ir.total_fees || 0); // SIGNED value
      const r = Number(ir.refund_amount || 0);
      if (type === "Pesanan") {
        settleSum += s;
        feeSum += f; // Sum SIGNED values
        refSum += Math.abs(r);
    ...
    totals.platformFee = Math.round(Math.abs(feeSum)); // ABS of SUM
    totals.platformFeeFinal = Math.round(Math.abs(feeSum));
    ...
    // Net = Omzet Kotor (gross) - Diskon Seller
    // PENTING: totals.omzet menyimpan nilai setelah diskon, gunakan totals.gross untuk hitung net
    const omzetNetForHeld = Math.max(0, (totals.gross || 0) - (totals.sellerDiscount || 0));
    totals.held = Math.max(0, omzetNetForHeld - totals.platformFee - totals.settlement);
  ...
  // RECOMPUTE adSpend total = GMV settlement + Top Up
  totals.adSpend = (totals.adSpendSettlement || 0) + (totals.adSpendTopup || 0);

  // === RECOMPUTE PROFIT using correct formula per mode ===
  // PENTING: totals.omzet sudah menyimpan nilai net (setelah diskon seller dari orderTotal)
  // totals.gross = Omzet Kotor (sebelum diskon)
  // Omzet Net (UI) = totals.gross - totals.sellerDiscount
  const finalOmzetNet = Math.max(0, (totals.gross || 0) - (totals.sellerDiscount || 0));
  totals.net = finalOmzetNet; // expose ke API sebagai field 'net'

  totals.profitBeforeAds = totals.profit; // Keep original per-group profit
  if (calcMode === "settlement" && incomeRawRows.length > 0) {
    // Settlement mode: profit based on actual cash received
    ...
    // Accrual mode: profit = omzetNet - potonganPlatform - refund + penggantian - hpp - packing - iklan
    ...
    const iklanGMVTotal = totals.adSpendSettlement || 0;
    const iklanTopUpTotal = totals.adSpendTopup || 0;
    const netTransaksi = finalOmzetNet - platformFee - refund + penggantian;
  ...
  // finalAdSpend = TOTAL iklan (GMV settlement + Top Up)
  totals.finalAdSpend = totals.adSpendSettlement + totals.adSpendTopup;
  totals.ads = totals.finalAdSpend; // expose sebagai 'ads' untuk front-end
  ...
  // Margin = profit / omzetNet (= gross - sellerDiscount)
  // PENTING: pakai totals.net sebagai denominator tunggal untuk konsistensi
  // Menggunakan let karena margin akan di-recompute setelah final override profit di bawah
  const netBase = totals.net || totals.gross || totals.omzet || 0;
  let margin = netBase ? totals.profit / netBase * 100 : 0;
  let finalMargin = netBase ? totals.finalProfit / netBase * 100 : 0;
  const estimatedMargin = netBase ? totals.estimatedProfit / netBase * 100 : 0;
  let bookMargin = netBase ? totals.bookProfit / netBase * 100 : 0;
  // BUG FIX: Hapus override profit/omzet per toko yang salah (dulu semua toko dapat profit total yang sama).
  // Per-store profit sudah diakumulasi dengan benar di loop groups di atas.
  ...
  if (missingForAlert.size) alerts.push({ level: "warn", title: "Ada SKU tanpa HPP", body: `${missingForAlert.size} SKU belum punya HPP.` });
  ...
  const counts = {
    ...
    // BUG FIX: eligibleAccrualOrders = Selesai + Dikirim (finalOrders + estimatedOrders)
    // Sebelumnya salah pakai heldOrders (hanya yang belum cair)
    eligibleAccrualOrders: totals.finalOrders.size + totals.estimatedOrders.size,
    pendingSettlementOrders: totals.pendingSettlementOrders.size,
    pendingSettlementOmzet: Math.round(totals.pendingSettlementOmzet || 0),
  ...
  const sortedOperationDetails = operationDetails
    .sort((a, b) =>
      // 1st: pending settlement (untuk drilldown Dana Tertahan, harus kelihatan)
      Number(b.pendingSettlement) - Number(a.pendingSettlement) ||
      // 2nd: late orders
      Number(b.late) - Number(a.late) ||
      // 3rd: newest first
      Number(b.ageDays) - Number(a.ageDays) ||
  ...
  if (incomeRawRows && incomeRawRows.length > 0) {
    ...
    for (const ir of incomeRawRows) {
      ...
      if (t === 'Pesanan') {
        ...
        if (validOrderIds.has(String(ir.order_id || '').trim())) {
          sSum += amt;
          fSum += Number(ir.total_fees || 0); // Sum SIGNED values, ABS later
        }
      }
    ...
    if (sSum !== 0) {
      ...
      totals.platformFee = Math.round(Math.abs(fSum)); // ABS of signed sum
      totals.platformFeeFinal = Math.round(Math.abs(fSum));
      totals.platformFeeEstimated = 0;
      ...
      totals.finalProfit = totals.profit;
      // Recompute adSpend setelah override
      totals.finalAdSpend = totals.adSpendSettlement + totals.adSpendTopup;
      totals.ads = totals.finalAdSpend;
    }
  }

  // === RECOMPUTE MARGIN di sini (setelah semua profit/net final) ===
  // HARUS di sini, setelah final override block, agar profit yang digunakan sudah final
  {
    const netBase = totals.net || totals.gross || totals.omzet || 0;
    margin = netBase ? totals.profit / netBase * 100 : 0;
    finalMargin = netBase ? (totals.finalProfit || totals.profit) / netBase * 100 : 0;
    bookMargin = netBase ? (totals.bookProfit || totals.profit) / netBase * 100 : 0;
  }
  ...
  const result = {
    ...
    data_quality_status: incomeRawRows.length === 0 ? "DATA_BELUM_LENGKAP" :
      (totals.pendingSettlementOrders && totals.pendingSettlementOrders.size > 20 ? "ESTIMASI" : "VALID"),
    // ── NEW: compute availability flags as locals first ──
    // ── dataset availability: import_runs OR actual data fallback ──
    incomeAvailable: (() => { const fromRuns = filters.store !== 'all' && runs.some(r => r.store_name === filters.store && r.kind === 'income'); return fromRuns || incomeRawRows.length > 0; })(),
    adsAvailable: (() => { const fromRuns = filters.store !== 'all' && runs.some(r => r.store_name === filters.store && (r.kind === 'ads' || r.kind === 'ad_spend')); return fromRuns || adRowsAll.length > 0; })(),
    // BUG FIX: returnAvailable berdasarkan data order aktual (bukan hanya import runs 'returns' yang tidak pernah ada)
    returnAvailable: orderLines.some(r => isReturnStatus(r.status, r.cancel_reason)) || orderLines.some(r => isCancelStatus(r.status)),
    // ── NEW: HPP completeness ──
    hppCompleteness: missingCost.size > 0 ? "INCOMPLETE" : "COMPLETE",
    missingHppSkuCount: missingCost.size,
    missingHppSkus: Array.from(missingCost).slice(0, 30),
    // ── NEW: profit validity ──
    profitValidity: (missingCost.size > 0 || incomeRawRows.length === 0) ? "BELUM_VALID" : "VALID",
    profitValidityReasons: [
      ...(missingCost.size > 0 ? ["HPP_BELUM_LENGKAP"] : []),
      ...(!(filters.store !== 'all' && runs.some(r => r.store_name === filters.store && r.kind === 'income')) ? ["INCOME_BELUM_DIUPLOAD"] : []),
      ...(!(filters.store !== 'all' && runs.some(r => r.store_name === filters.store && (r.kind === 'ads' || r.kind === 'ad_spend'))) ? ["ADS_BELUM_DIUPLOAD"] : []),
      ...(!(filters.store !== 'all' && runs.some(r => r.store_name === filters.store && r.kind === 'returns')) ? ["RETURN_BELUM_DIUPLOAD"] : []),
    ],
    // ── NEW: period coverage ──
    dataCoverageStart: orderLines.length > 0 ?
      orderLines.reduce((min, r) => { const d = String(r.created_at || '').slice(0, 10); return (!min || d < min) ? d : min; }, '') : null,
    dataCoverageEnd: orderLines.length > 0 ?
      orderLines.reduce((max, r) => { const d = String(r.created_at || '').slice(0, 10); return (!max || d > max) ? d : max; }, '') : null,
    // ── NEW: store config ──
    storePackingConfig: stores.size > 0 ? { method: 'PER_PACKAGE', amount: 2000 } : null,
    filters: { ...filters, startDate, endDate },
    availableMonths,
    availableStores: configuredStores((await readConfig().catch(() => ({}))).stores || DEFAULT_STORES),
  ...
  // ── Store in cache ──
  pg.cacheSet(cacheKey, result).catch(() => { });

  // ── GATE: Nullify financial metrics when critical data is missing ──
  if (incomeRawRows.length === 0) {
    result.totals.held = null;
    result.totals.heldOrders = null;
    result.totals.pendingSettlementOrders = 0;
    result.totals.pendingSettlementOmzet = 0;
    result.totals.platformFee = null;
    result.totals.platformFeeFinal = null;
    result.totals.settlement = null;
    result.totals.adjustmentAmount = null;
    result.totals.settlementAdSpend = null;
    result.totals.adSpendSettlement = null;
  }
  // Nullify profit hanya jika data benar-benar tidak cukup untuk kalkulasi akurat:
  // - Ada SKU tanpa HPP (missingCost)
  // - Tidak ada income data sama sekali (settlement tidak bisa dihitung)
  // TIDAK nullify karena tidak ada ads — profit tetap valid, iklan = 0
  if (missingCost.size > 0 || incomeRawRows.length === 0) {
    result.totals.profit = null;
    result.totals.margin = null;
    result.totals.finalProfit = null;
    result.totals.bookProfit = null;
  }
...
async function computeProductProfitability(filters = { preset: "month", month: "", store: "custombase", mode: "accrual" }) {
  const startedAt = Date.now();
  if (!supabaseConfigured()) return { products: [], reconciliation: null, version: CALCULATION_VERSION };

  function ct(v) { return String(v || '').toLowerCase().replace(/[^a-z0-9]+/g, ''); }

  const [startDate, endDate] = dateRangeFromFilters(filters);
  const storeFilter = filters.store !== "all" ? filters.store : "custombase";
  const orderQuery = compactQuery(["select=*", `store_name=eq.${encodeURIComponent(storeFilter)}`, dateAndFilter("created_at", startDate, endDate)]);
  const incomeQuery = compactQuery(["select=*", `store_name=eq.${encodeURIComponent(storeFilter)}`, dateAndFilter("order_created_time", startDate, endDate)]);

  const [orderLines, skuCosts, incomeRawRows] = await Promise.all([
    fetchAll("finance_order_lines", orderQuery),
    fetchAll("finance_sku_costs", "select=*"),
    fetchAll("finance_income_raw", incomeQuery),
  ]);

  // Build SKU cost map — SAMA dengan computeSummary: store-specific > global > any-store
  const ppCostsStore = new Map();  // "storeName|sku" → row
  const ppCostsGlobal = new Map(); // "sku" → row (store_name='global')
  const ppCostsAny = new Map();    // "sku" → row (any store yang punya HPP)
  for (const s of skuCosts) {
    const skuLower = ct(s.sku);
    const storeLower = String(s.store_name || '').toLowerCase();
    if (storeLower === 'global' || storeLower === '' || storeLower === 'all') {
      if (!ppCostsGlobal.has(skuLower)) ppCostsGlobal.set(skuLower, s);
    } else {
      const storeKey = storeLower + '|' + skuLower;
      if (!ppCostsStore.has(storeKey)) ppCostsStore.set(storeKey, s);
    }
    if (!ppCostsAny.has(skuLower) && Number(s.hpp_per_unit || 0) > 0) {
      ppCostsAny.set(skuLower, s);
    }
  }
  // Helper HPP: DB lookup (store-specific > global > any) lalu special hardcoded
  function getHpp(sku, variation, storeName) {
    const key = ct(sku);
    const storeKey = ct(storeName || '') + '|' + key;
    const cost = ppCostsStore.get(storeKey) || ppCostsGlobal.get(key) || ppCostsAny.get(key);
    if (cost && Number(cost.hpp_per_unit) > 0) return Number(cost.hpp_per_unit);
    // Fallback ke special SKU logic (polaroidnew variation-based, dll)
    return getSpecialSkuHpp(key, variation);
  }

  function getPacking(sku, variation, storeName) {
    const key = ct(sku);
    const storeKey = ct(storeName || '') + '|' + key;
    const cost = ppCostsStore.get(storeKey) || ppCostsGlobal.get(key) || ppCostsAny.get(key);
    if (cost && Number(cost.packing_per_unit) > 0) return Number(cost.packing_per_unit);
    // Fallback ke special packing logic (polaroidnew = 2000/paket)
    return getSpecialSkuPacking(key, variation);
  }
  ...
  // Build income match set
  const incomeOids = new Set();
  const incomeByOid = new Map();
  for (const ir of incomeRawRows) {
    if (String(ir.transaction_type || '').trim() !== 'Pesanan') continue;
    const oid = String(ir.order_id || '').trim();
    incomeOids.add(oid);
    if (!incomeByOid.has(oid)) incomeByOid.set(oid, []);
    incomeByOid.get(oid).push(ir);
  }

  // Build return data
  const returnsByOid = new Map();
  try {
    const rets = await fetchAll("finance_returns", `store_name=eq.${encodeURIComponent(storeFilter)}`);
    for (const r of rets) {
  ...
  const calcMode = filters.mode || 'accrual';
  const skuMap = new Map(); // sku → aggregated data

  for (const [oid, rows] of orderGroups) {
    const first = rows[0];
    const st = ct(first.status || '');
    ...
    const isCancelled = st.includes('dibatalkan');
    const cancelReason = ct(first.cancel_reason || '');
    const isCancelValid = isCancelled && (cancelReason.includes('pengirimanpaketgagal') || cancelReason.includes('pakethilang'));

    const eligible = calcMode === 'settlement' ? isSelesai : (isSelesai || isDikirim);
    if (!eligible && !isCancelValid) continue;

    // Per-order totals
    let orderGross = 0, orderDiscount = 0, orderNet = 0;
    for (const r of rows) {
      orderGross += Math.abs(Number(r.gross_product || 0));
      orderDiscount += Math.abs(Number(r.seller_discount || 0));
    }
    orderNet = Math.max(0, orderGross - orderDiscount);

    // Packing key
    const pkgKey = rows.map(r => String(r.tracking_id || r.package_id || r.order_id || '').trim()).find(Boolean) || oid;

    // Income data for this order
    const incRows = incomeByOid.get(oid) || [];
    let orderFeeSigned = 0, orderSettlement = 0;
    for (const ir of incRows) {
      orderFeeSigned += Number(ir.total_fees || 0);
      orderSettlement += Number(ir.settlement_amount || 0);
    }
    const orderFee = Math.abs(orderFeeSigned);

    // Returns
    const rets = returnsByOid.get(oid) || [];
    const hasReturnRisk = rets.some(r => r.return_status === 'To Process' || r.return_status === 'In Process');
    const completedRets = rets.filter(r => r.return_status === 'Completed');

    // Dana Tertahan check
    const hasIncome = incRows.length > 0 && orderSettlement > 0;
    const isDT = (isDikirim || isSelesai) && !hasIncome && !isCancelled;
    ...
    for (const r of rows) {
      ...
      if (!skuMap.has(skuKey)) {
        skuMap.set(skuKey, {
          ...
          hpp: 0,
          packingAllocated: 0,
          penggantianAttributed: 0,
          adCostAttributed: 0,
          refundOrderCount: new Set(),
          cancelOrderCount: new Set(),
          returnPendingCount: 0,
          outstandingReceivable: 0,
          returnRiskReceivable: 0,
          hasHpp: false,
          dataQualityFlags: [],
        });
      }
      ...
      const lineGross = Math.abs(Number(r.gross_product || 0));
      const lineDiscount = Math.abs(Number(r.seller_discount || 0));
      const lineNet = Math.max(0, lineGross - lineDiscount);
      const share = orderNet > 0 ? lineNet / orderNet : 1 / Math.max(rows.length, 1);

      item.unitsSold += qty;
      item.uniqueOrders.add(oid);
      item.uniquePackages.add(pkgKey);
      item.omzetKotor += lineGross;
      item.diskonSeller += lineDiscount;
      item.omzetNet += lineNet;

      // HPP — cross-store lookup
      const hpp = getHpp(sku, r.variation, r.store_name);
      if (hpp > 0) item.hasHpp = true;
      item.hpp += qty * hpp;

      // Packing — DB packing_per_unit atau special fallback (polaroidnew = 2000/paket × qty)
      const packingUnit = getPacking(sku, r.variation, r.store_name);
      item._rawPacking = (item._rawPacking || 0) + (qty * packingUnit);
      item._rawFee = (item._rawFee || 0) + (Math.abs(orderFeeSigned) * share);

      // Refund
      const refundAmt = Math.abs(Number(r.refund_amount || 0));
      item.refundSellerBasis += refundAmt;
      if (refundAmt > 0) item.refundOrderCount.add(oid);

      if (isCancelled) item.cancelOrderCount.add(oid);

      // Dana Tertahan
      if (isDT) {
        item.outstandingReceivable += lineNet;
        if (hasReturnRisk) item.returnRiskReceivable += lineNet;
      }

      if (hasReturnRisk) item.returnPendingCount++;
    }
  }

  // Convert to array, apply residual rounding, compute derived metrics
  const products = [];

  // Residual rounding for packing — use per-package count as target  
  let targetPacking = 0;
  for (const [oid, rows] of orderGroups) {
    const first = rows[0]; const st = ct(first.status || '');
    if (!st.includes('selesai') && !(st === 'dikirim' || (st.includes('dikirim') && !st.includes('perlu')))) {
      const cr = ct(first.cancel_reason || ''); const isCV = st.includes('dibatalkan') && (cr.includes('pengirimanpaketgagal') || cr.includes('pakethilang'));
      if (!isCV) continue;
    }
    targetPacking += 2000; // Per-order packing (simplified — matches store when no shared packages)
  }
  // Use raw accumulated sum for residual distribution target (handles float perfectly)
  const rawPackingSum = Math.round([...skuMap.values()].reduce((s, item) => s + (item._rawPacking || 0), 0));
  const packingTarget = rawPackingSum; // Raw sum is the true allocation total
  let packingFloors = [];
  let floorSum = 0;
  for (const [key, item] of skuMap) {
    const floor = Math.floor(item._rawPacking || 0);
    packingFloors.push({ key, floor, frac: (item._rawPacking || 0) - floor });
    floorSum += floor;
  }
  let packingResidual = packingTarget - floorSum;
  packingFloors.sort((a, b) => b.frac - a.frac);
  for (const pf of packingFloors) {
    const alloc = pf.floor + (packingResidual > 0 ? 1 : 0);
    if (packingResidual > 0) packingResidual--;
    skuMap.get(pf.key).packingAllocated = alloc;
  }

  // Residual rounding for platform fee — use actual store fee as target
  const targetFee = Math.abs([...incomeByOid.values()].flat().reduce((s, ir) => s + Number(ir.total_fees || 0), 0));
  let feeFloors = [];
  let feeFloorSum = 0;
  for (const [key, item] of skuMap) {
    const floor = Math.floor(item._rawFee || 0);
    feeFloors.push({ key, floor, frac: (item._rawFee || 0) - floor });
    feeFloorSum += floor;
  }
  let feeResidual = targetFee - feeFloorSum;
  feeFloors.sort((a, b) => b.frac - a.frac);
  for (const ff of feeFloors) {
    const alloc = ff.floor + (feeResidual > 0 ? 1 : 0);
    if (feeResidual > 0) feeResidual--;
    skuMap.get(ff.key).platformFeeActual = alloc;
  }

  for (const [key, item] of skuMap) {
    const orders = item.uniqueOrders.size;
    const hasHpp = item.hasHpp;
    const contributionBeforeAds = item.omzetNet - item.refundSellerBasis - item.platformFeeActual - item.hpp - item.packingAllocated;
    const contributionAfterAds = contributionBeforeAds - item.adCostAttributed;
    const margin = item.omzetNet > 0 ? (contributionBeforeAds / item.omzetNet) * 100 : 0;
    const refundRate = orders > 0 ? (item.refundOrderCount.size / orders) * 100 : 0;
    const cancelRate = orders > 0 ? (item.cancelOrderCount.size / orders) * 100 : 0;
    ...
    if (!hasHpp) {
      ...
      item.dataQualityFlags.push('SKU_BELUM_ADA_HPP');
    } else if (orders < 5) {
      status = 'BELUM CUKUP DATA';
    } else if (contributionBeforeAds <= 0) {
      status = 'LOSING';
      statusLevel = -1;
    } else if (margin >= 20 && orders >= 30 && refundRate <= 5) {
      status = 'WINNING';
      statusLevel = 3;
    } else if (contributionBeforeAds > 0 && orders < 30) {
      status = 'PROMISING';
      statusLevel = 2;
    } else if (margin > 0 && margin < 20) {
      status = 'WATCH';
      statusLevel = 1;
    } else if (contributionBeforeAds > 0 && (refundRate > 10 || cancelRate > 15)) {
      status = 'WATCH';
      statusLevel = 1;
    } else {
      status = 'WATCH';
    ...
    products.push({
      ...item,
      uniqueOrders: orders,
      uniquePackages: item.uniquePackages.size,
      contributionBeforeAds,
      contributionAfterAds,
      contributionMargin: Math.round(margin * 100) / 100,
      profitPerOrder: orders > 0 ? Math.round(contributionBeforeAds / orders) : 0,
      profitPerUnit: item.unitsSold > 0 ? Math.round(contributionBeforeAds / item.unitsSold) : 0,
      refundRate: Math.round(refundRate * 100) / 100,
      cancelRate: Math.round(cancelRate * 100) / 100,
      status,
      statusLevel,
      adAllocationStatus: 'UNAVAILABLE',
      dataQualityStatus: hasHpp ? 'VALID' : 'BELUM_VALID',
    });
  }

  // Sort by contribution
  products.sort((a, b) => b.contributionBeforeAds - a.contributionBeforeAds);

  // Store reconciliation
  const recon = {
    gross: Math.round(products.reduce((s, p) => s + p.omzetKotor, 0)),
    discount: Math.round(products.reduce((s, p) => s + p.diskonSeller, 0)),
    net: Math.round(products.reduce((s, p) => s + p.omzetNet, 0)),
    hpp: Math.round(products.reduce((s, p) => s + p.hpp, 0)),
    packing: Math.round(products.reduce((s, p) => s + p.packingAllocated, 0)),
    platformFee: Math.round(products.reduce((s, p) => s + p.platformFeeActual, 0)),
    refund: Math.round(products.reduce((s, p) => s + p.refundSellerBasis, 0)),
    contribution: Math.round(products.reduce((s, p) => s + p.contributionBeforeAds, 0)),
  };
  ...
  return {
    ...
    rankings: {
      topProfit: products.filter(p => p.statusLevel >= 1).slice(0, 10),
      topMargin: [...products].filter(p => p.uniqueOrders >= 5).sort((a, b) => b.contributionMargin - a.contributionMargin).slice(0, 10),
      topVolume: [...products].sort((a, b) => b.unitsSold - a.unitsSold).slice(0, 10),
      worstLoss: [...products].filter(p => p.contributionBeforeAds < 0).sort((a, b) => a.contributionBeforeAds - b.contributionBeforeAds).slice(0, 10),
      highestRefund: [...products].filter(p => p.uniqueOrders >= 5).sort((a, b) => b.refundRate - a.refundRate).slice(0, 10),
      highestCancel: [...products].filter(p => p.uniqueOrders >= 5).sort((a, b) => b.cancelRate - a.cancelRate).slice(0, 10),
      highestReturnRisk: [...products].filter(p => p.returnRiskReceivable > 0).sort((a, b) => b.returnRiskReceivable - a.returnRiskReceivable).slice(0, 10),
    },
    filters: { ...filters, startDate, endDate },
    calculation_version: CALCULATION_VERSION,
    generatedAt: nowIso(),
    diagnostics: { productCount: products.length, ms: Date.now() - startedAt },
  };
}
