...
async function importRows({ storeName, kind, filename, rows }) {
  ...
  if (detected === "ads") {
    ...
    for (const row of cleanRows) {
      ...
      // ── Fallback transaksi debit lainnya ──
      else if (amount > 0 && !fundType.toLowerCase().includes("credit")) {
        adRows.push({
          store_name: normalizeStore(storeName || DEFAULT_STORE),
          spend_date: spendDate, amount,
          channel: "TikTok Ads Lainnya", campaign: (type + "/" + subtype).replace(/^\//, ""),
          note: (description || txId).trim(),
          transaction_id: txId || null,
          status: status,
          created_at: nowIso(), updated_at: nowIso(),
        });
