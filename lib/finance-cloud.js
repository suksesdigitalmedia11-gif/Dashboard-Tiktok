...
async function importRows({ storeName, kind, filename, rows }) {
  ...
  if (detected === "ads") {
    ...
    for (const row of cleanRows) {
      ...
      if (type === "General" && subtype === "Add balance") {
        adRows.push({
      ...
      // Bill payment = GMV ad settlement (auto-deducted by TikTok)
      else if (type === "General" && subtype === "Bill payment") {
        adRows.push({
          store_name: normalizeStore(storeName || DEFAULT_STORE),
          spend_date: spendDate,
          amount,
          ...
          campaign: "GMV Auto",
          note: "GMV Bill: " + (field(row, "Transaction ID") || ""),
          created_at: nowIso(),
          updated_at: nowIso(),
        });
    ...
    await finishImportRun(run && run.id, {
      ...
      message: `Iklan diproses: ${adRows.length} transaksi (top-up + GMV: Rp${Math.round(gmvTotal).toLocaleString("id-ID")})`,
    });
...
async function computeSummary(filters = { preset: "thisMonth", month: "", store: "all", mode: "accrual" }) {
  ...
  for (const [groupKey, orderRows] of groups.entries()) {
    ...
    operationDetails.push({
      ...
      ageDays,
      cancelValid,
      pendingSettlement: false, // will be updated below if applicable
    });
    ...
    if (!opCancelOnly && !opUnpaid && (isCompleted || isShipped) && !hasSettlement && !hasIncomeMatch) {
      ...
      // Also flag in operationDetails for drilldown
      if (operationDetails.length > 0) operationDetails[operationDetails.length - 1].pendingSettlement = true;
    }
