...
async function computeSummary(filters = { preset: "thisMonth", month: "", store: "all", mode: "accrual" }) {
  ...
  for (const [groupKey, orderRows] of groups.entries()) {
    ...
    // BUG FIX: Packing = SUM(quantity * packing_per_unit) per SKU baris, bukan flat rate per paket
    // Fallback: jika TIDAK ADA packing di DB sama sekali, pakai Rp 2000/paket sebagai default aman
    const hasPackingFromDb = financialRows.some(r => Number(r.packing_per_unit || 0) > 0);
    const packingTotal = hasPackingFromDb
      ? financialRows.reduce((sum, r) => sum + Number(r.quantity || 0) * Number(r.packing_per_unit || 0), 0)
      : packageCount * 2000;
