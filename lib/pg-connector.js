...
async function pgQuery(sql, params = []) {
  ...
  if (mode === 'turso' && turso) {
    try {
      ...
      return { rows: r.rows || [], changes: r.rowsAffected || 0 };
...
async function fetchAll(table, queryStr = '') {
  ...
  if (mode === 'turso' && turso) {
    try { const s = normalizeSql(sql); const r = await turso.execute(s); return r.rows || []; }
    catch (e) { console.error('[db] fetchAll error:', e.message.substring(0, 80)); return []; }
  }
...
async function initSchema() {
  ...
  if (mode === 'postgres' && pool) {
    const pgDdl = ddl
      ...
      .replace(/TEXT PRIMARY KEY/g, 'TEXT'); // keep TEXT PK as-is (PG supports it)
    await pool.query(pgDdl);
    for (const idx of indexes.slice(0, 3)) { try { await pool.query(idx); } catch (e) { } }
    try { await pool.query("CREATE UNIQUE INDEX IF NOT EXISTS idx_income_unique ON finance_income_raw(store_name, order_id, transaction_type)"); } catch (e) { }
