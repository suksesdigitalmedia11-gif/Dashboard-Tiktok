...
async function upsertAdSpendRows(rows) {
  if (!rows || !rows.length) return [];
  const BATCH = 200;
  // transaction_id WAJIB ada untuk deduplication yang benar.
  // Tanpa transaction_id, re-upload file yang sama akan INSERT rows duplikat
  // karena unique index (store_name, spend_date, channel, campaign) terlalu lebar.
  const cols = ["store_name", "spend_date", "amount", "channel", "campaign", "note", "transaction_id", "status", "created_at", "updated_at"];
  ...
  // SQLite path: INSERT OR IGNORE per transaction_id agar setiap transaksi tersimpan terpisah.
  // Sebelumnya pakai upsertRows(conflict=store+date+channel+campaign) yang terlalu broad:
  // per hari per campaign hanya 1 baris tersimpan, kehilangan semua transaksi berikutnya.
  if (mode === 'sqlite' && db) {
    const insertStmt = db.prepare(
      `INSERT OR IGNORE INTO finance_ad_spend(store_name,spend_date,amount,channel,campaign,note,transaction_id,status,created_at,updated_at)
       VALUES (?,?,?,?,?,?,?,?,?,?)`
    );
    const insertMany = db.transaction((items) => {
      for (const r of items) {
        insertStmt.run(
          r.store_name, r.spend_date, r.amount,
          r.channel || '', r.campaign || '', r.note || '',
          r.transaction_id || null, r.status || null,
...
async function initSchema() {
  ...
  const indexes = [
    ...
    "CREATE INDEX IF NOT EXISTS idx_ad_txid ON finance_ad_spend(transaction_id)",
  ];
